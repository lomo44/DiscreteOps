...
class test_vrp_local_search(unittest.TestCase):
    def setUp(self):
        self.problem_context = parse_input_data_from_file("./data/vrp_421_41_1")
        self.cache = vrp_cache()
        self.cache.generate_cache(self.problem_context)
    ...
    def test_greedy_solution(self):
        ...
        self.assertEqual(self.greedy_solution.current_cost, get_solution_cost(self.problem_context,self.greedy_solution,self.cache))
    def test_swap(self):
        for i in range(0,100):
            ...
            swap_context.apply_changes(self.greedy_solution)
            self.assertTrue(check_solution_valid(self.problem_context,self.greedy_solution))
            self.assertEqual(self.greedy_solution.current_cost-current_cost, swap_context.cost_delta)
    def test_move(self):
        for i in range(0,100):
            swap_context = vrp_local_search_context(vrp_local_search_type.vrp_move)
            current_cost = self.greedy_solution.current_cost
            swap_context.get_estimate(self.greedy_solution,self.problem_context,self.cache)
            swap_context.apply_changes(self.greedy_solution)
            self.assertTrue(check_solution_valid(self.problem_context,self.greedy_solution))
